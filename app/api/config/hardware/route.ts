...
// POST /api/config/hardware - Create a new hardware item
export async function POST(request: NextRequest) {
  try {
    ...
    // Only admin can create hardware items
    ...
    const result = await pool.query(
      `INSERT INTO hardware_items 
       ...
       RETURNING id, name, cost, manager_cost as "managerCost", user_cost as "userCost", 
                 quantity, locked, is_extension as "isExtension", is_active as "isActive", 
                 display_order as "displayOrder", created_at as "createdAt", updated_at as "updatedAt"`,
      [name, cost, managerCost, userCost, quantity, locked, isExtension, displayOrder]
    );
...
// PUT /api/config/hardware - Update a hardware item
...
// DELETE /api/config/hardware - Soft delete a hardware item
