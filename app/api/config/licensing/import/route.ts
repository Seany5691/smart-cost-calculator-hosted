...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      try {
        ...
        const existingResult = await query(
          'SELECT id, display_order FROM licensing_items WHERE LOWER(name) = LOWER($1) AND is_active = true',
          [name]
        );
        ...
        if (existingResult.rows.length > 0) {
          ...
          const maxOrderResult = await query(
            'SELECT COALESCE(MAX(display_order), -1) as max_order FROM licensing_items WHERE is_active = true'
          );
          const displayOrder = maxOrderResult.rows[0].max_order + 1;

          // Create new item
          await query(
            `INSERT INTO licensing_items (name, cost, manager_cost, user_cost, quantity, locked, is_active, display_order, created_at, updated_at)
    ...
    try {
      await query('UPDATE config_cache SET updated_at = NOW() WHERE config_type = $1', ['licensing']);
    } catch (error) {
    ...
    console.error('Licensing import error:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to import licensing configuration' },
      { status: 500 }
    );
  }
}
