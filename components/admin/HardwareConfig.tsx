...
import { exportHardwareToExcel } from '@/lib/admin/excel-export';
...
export default function HardwareConfig() {
  const { hardware, fetchHardware } = useConfigStore();
  const [items, setItems] = useState<HardwareItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editForm, setEditForm] = useState<Partial<HardwareItem>>({});
  ...
  const loadItems = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setItems(typedData.sort((a: HardwareItem, b: HardwareItem) => a.displayOrder - b.displayOrder));
      }
    } catch (error) {
      console.error('Failed to load hardware items:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdate = async (id: string) => {
    try {
      const response = await fetch(`/api/config/hardware/${id}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/config/hardware/${deleteItemId}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
  ...
  const applyBulkMarkup = async (managerMarkup: string, userMarkup: string) => {
    try {
      ...
      for (const item of updatedItems) {
        await fetch(`/api/config/hardware/${item.id}`, {
          method: 'PUT',
          headers: getAuthHeaders(),
          body: JSON.stringify({
  ...
  const handleExport = () => {
    try {
      exportHardwareToExcel(items);
    } catch (error) {
      console.error('Failed to export hardware:', error);
      setAlertConfig({
        title: 'Export Error',
        message: 'Failed to export hardware configuration',
        type: 'error'
      });
      setShowAlertModal(true);
    }
  ...
  const handleImportComplete = async () => {
    ...
    setAlertConfig({
      ...
      message: 'Hardware configuration imported successfully',
      type: 'success'
    });
    setShowAlertModal(true);
  };
  ...
  const handleMoveUp = async (index: number) => {
    ...
    for (let i = 0; i < newItems.length; i++) {
      await fetch(`/api/config/hardware/${newItems[i].id}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
    ...
    return (
      <div className="space-y-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-white">Hardware Items</h2>
        ...
        {showAddForm && (
          <div className="bg-white/20 p-4 rounded-lg space-y-3">
            ...
            <label className="flex items-center text-white">
              <input
                type="checkbox"
                checked={editForm.isExtension || false}
                onChange={(e) => setEditForm({ ...editForm, isExtension: e.target.checked })}
                className="mr-2"
              />
              Is Extension
            </label>
            ...
            {editingId === item.id ? (
              <>
                ...
                <label className="flex items-center text-white">
                  <input
                    type="checkbox"
                    checked={editForm.isExtension !== undefined ? editForm.isExtension : item.isExtension}
                    onChange={(e) => setEditForm({ ...editForm, isExtension: e.target.checked })}
                    className="mr-2"
                  />
                  Is Extension
              ...
              <>
                ...
                <div className="text-sm text-slate-300">
                  {item.isExtension && <span className="bg-purple-500 px-2 py-1 rounded mr-2">Extension</span>}
        ...
        <table className="w-full text-white">
          <thead>
            <tr className="border-b border-white/20">
              ...
              <th className="text-center py-3 px-4">Extension</th>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex flex-col space-y-1">
                        <button
                          ...
                          disabled={index === 0}
                          className="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed text-xs"
                        >
                          ↑
                        </button>
                        <button
                          onClick={() => handleMoveDown(index)}
                          disabled={index === items.filter(i => i.isActive).length - 1}
                          className="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed text-xs"
                        >
                          ↓
                        </button>
