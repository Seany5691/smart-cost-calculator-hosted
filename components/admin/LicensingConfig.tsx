...
import { exportLicensingToExcel } from '@/lib/admin/excel-export';
...
export default function LicensingConfig() {
  const { licensing, fetchLicensing } = useConfigStore();
  const [items, setItems] = useState<LicensingItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editForm, setEditForm] = useState<Partial<LicensingItem>>({});
  ...
  const loadItems = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setItems(typedData.sort((a: LicensingItem, b: LicensingItem) => a.displayOrder - b.displayOrder));
      }
    } catch (error) {
  ...
  const handleUpdate = async (id: string) => {
    try {
      const response = await fetch(`/api/config/licensing/${id}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/config/licensing/${deleteItemId}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
  ...
  const applyBulkMarkup = async (managerMarkup: string, userMarkup: string) => {
    try {
      ...
      for (const item of updatedItems) {
        await fetch(`/api/config/licensing/${item.id}`, {
          method: 'PUT',
          headers: getAuthHeaders(),
          body: JSON.stringify({
  ...
  const handleExport = () => {
    try {
      exportLicensingToExcel(items);
    } catch (error) {
      console.error('Failed to export licensing:', error);
      setAlertConfig({
        title: 'Export Error',
        message: 'Failed to export licensing configuration',
        type: 'error'
      });
      setShowAlertModal(true);
    }
  ...
  const handleImportComplete = async () => {
    ...
    setAlertConfig({
      ...
      message: 'Licensing configuration imported successfully',
      type: 'success'
    });
    setShowAlertModal(true);
  };
    ...
    return (
      <div className="space-y-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-white">Licensing Items</h2>
