 ...
 */
export default function ConnectivityBreakdown({ connectivity, dealId, isAdmin, onCostUpdate }: ConnectivityBreakdownProps) {
  ...
  const handleSave = (itemName: string) => {
    ...
    if (!isNaN(value) && value >= 0) {
      onCostUpdate('connectivity', itemName, value);
  ...
  const handleReset = (itemName: string) => {
    onCostUpdate('connectivity', itemName, null); // null means reset to original
    setEditingItemIndex(null);
    setEditValue('');
  };
