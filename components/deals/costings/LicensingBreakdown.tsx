 ...
 */
export default function LicensingBreakdown({ licensing, dealId, isAdmin, onCostUpdate }: LicensingBreakdownProps) {
  ...
  const handleSave = (itemName: string) => {
    ...
    if (!isNaN(value) && value >= 0) {
      onCostUpdate('licensing', itemName, value);
  ...
  const handleReset = (itemName: string) => {
    onCostUpdate('licensing', itemName, null); // null means reset to original
    setEditingItemIndex(null);
    setEditValue('');
  };
