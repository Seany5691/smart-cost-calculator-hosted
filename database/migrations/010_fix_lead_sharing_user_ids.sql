-- Fix Lead Sharing Tables - Change user_id columns from INTEGER to VARCHAR for UUID support

-- Drop existing tables (they will be recreated with correct types)
DROP TABLE IF EXISTS lead_share_notifications;
DROP TABLE IF EXISTS reminder_shares;
DROP TABLE IF EXISTS lead_shares;

-- Recreate with correct data types (all IDs as VARCHAR for UUID support)
