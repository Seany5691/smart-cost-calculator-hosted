...
export default function HardwareConfig() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      {/* Desktop Table View */}
      <div className="hidden md:block card p-0 sm:p-6">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="text-left py-3 px-4 font-semibold text-sm">Cost Price</th>
                <th className="text-left py-3 px-4 font-semibold text-sm">Manager Cost</th>
                <th className="text-left py-3 px-4 font-semibold text-sm">User Cost</th>
                <th className="text-left py-3 px-4 font-semibold text-sm">Extension</th>
                <th className="text-left py-3 px-4 font-semibold text-sm">Locked</th>
                ...
                <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4">
                    {editingItem === item.id ? (
                      ...
                      <span className="text-sm">{formatCurrency(item.managerCost || item.cost)}</span>
                    )}
                  </td>
                  <td className="py-3 px-4">
                    {editingItem === item.id ? (
                      ...
                      <span className="text-sm">{formatCurrency(item.userCost || item.cost)}</span>
                    )}
                  </td>
                  <td className="py-3 px-4">
                    {editingItem === item.id ? (
      ...
      <div className="md:hidden space-y-3">
        {items.map((item, index) => (
          <div key={item.id} className="bg-white/60 backdrop-blur-sm rounded-lg p-3 shadow-sm border border-gray-200">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-xs ${
                    item.isExtension ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'
                  }`}>
                    {item.isExtension ? 'Extension' : 'Standard'}
                  </span>
                  <span className={`px-2 py-0.5 rounded-full text-xs ${
            ...
            {editingItem === item.id && (
              <div className="grid grid-cols-2 gap-3 mb-3 p-2 bg-blue-50 rounded-lg">
                <label className="flex items-center space-x-2">
                  <input
                    ...
                    className="w-4 h-4 text-blue-600 touch-manipulation"
                  />
                  <span className="text-xs text-gray-700">Extension</span>
                </label>
                <label className="flex items-center space-x-2">
                  <input
                    type="checkbox"
